...
class DojoBuilder<DRoom extends DojoRoom, DState> extends StatelessWidget {
  /// default constructor
  const DojoBuilder({
    Key? key,
  ...
  Widget build(BuildContext context) {
    return Builder(builder: (context) {
      final gym = Dojo.read<DRoom, StateToTest>(context);
...
/// inherited widget
class DojoGym<DState> extends InheritedWidget {
  const DojoGym({
    super.key,
  ...
  final DojoRoom<DState> dojoRoom;

  static DojoRoom of<DState>(BuildContext context) {
    final gym = context.dependOnInheritedWidgetOfExactType<DojoGym<DState>>();
    if (gym == null) {
      //THROW
    }
...
class DojoProvider<DState> extends StatefulWidget {
  const DojoProvider({
    super.key,
    required this.dojo,
    required this.child,
  ...
  final DojoRoom<DState> dojo;
  final Widget child;

  @override
  State<DojoProvider<DState>> createState() => _DojoProviderState<DState>();
}

class _DojoProviderState<DState> extends State<DojoProvider<DState>> {
...
class Dojo {
  ...
  static R read<R extends DojoRoom, DState>(BuildContext context) {
    return DojoGym.of<DState>(context) as R;
  }
